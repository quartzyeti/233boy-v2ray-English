...
api() {
    [[ $is_core_ver_lt_5 ]] && {
        warn "$is_core_ver version does not support API operations. Please upgrade the core version: $is_core update core"
        is_api_fail=1
        return
    }
...
main() {
    ...
    api | bin | convert | tls | run | uuid)
        [[ $is_core_ver_lt_5 ]] && {
            warn "$is_core_ver version does not support the use of commands. Please upgrade the kernel version: $is_core update core"
            return
        }
        is_run_command=$1
        if [[ $1 == 'bin' ]]; then
